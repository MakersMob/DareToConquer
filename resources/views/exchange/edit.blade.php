		...
		<div class="row">
			<div class="col-12 col-lg-6">
				...
				<div class="card">
					...
					<div class="card-body">
						{!! Form::open(['url' => 'exchange/'.$exchange->id, 'method' => 'patch']) !!}
							<div class="form-group">
								<label for="type">Task Type</label>
								<select class="form-control" name="type" id="type">
									<option value="link" @if($exchange->type == 'link') selected @endif>Backlink</option>
									<option value="facebook" @if($exchange->type == 'facebook') selected @endif>Facebook (Like/Share/Comment)</option>
									<option value="comment" @if($exchange->type == 'comment') selected @endif>Blog (Comment)</option>
									<option value="twitter" @if($exchange->type == 'twitter') selected @endif>Twitter (Reply/Retweet)</option>
									<option value="instagram" @if($exchange->type == 'instagram') selected @endif>Instagram (Like/Comment)</option>
									<option value="pinterest" @if($exchange->type == 'pinterest') selected @endif>Pinterest (Repin)</option>
								</select>
							</div>
							<div class="form-group">
								<label for="url">Task URL. Where can they find the location of the task?</label>
								<input type="text" name="url" id="url" class="form-control" value="{{ $exchange->url }}" required>
							</div>
							<div class="form-group">
								<label for="description">Task Description. What would you like them to do?</label>
								<textarea name="description" id="description" cols="" rows="10" class="form-control">{{ $description }}</textarea>
							</div>
